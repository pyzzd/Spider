...
def res_index():
    url ='https://www.lacasadelhabano-basel.ch/'
    try:
...
if __name__ == '__main__':
    ...
    if os.path.exists('spider01.txt'):
        with open('spider01.txt', 'r', encoding='utf-8') as f:
            dict = f.read().replace("'",'"')
            print(dict)
            dict = json.loads(dict)
    else:
        f = open('spider01.txt', 'w', encoding='utf-8')
        f.close()
    while True:
        print('监控中================')
        text = ''
        try:
            main()
            with open('spider01.txt','w',encoding='utf-8')as f:
                f.write(str(dict))
        except:
            time.sleep(5)
        time.sleep(10)
