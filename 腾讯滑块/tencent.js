...
window.localStorage = {
    "AEGIS_ID": "10110110-0011-0010-0011-111011011101",
    "CLOUD_COMMUNITY_MOBILE_MENU_TOOLTIP": "1",
    "QCLOUD_DEVELOPER_DEVICE_ID": "j2NzAoDqp1YYScZeb1D_E",
    "bookmarkOfCourse3871For100020313782": JSON.stringify({"contentId": 67334, "done": null}),
    "bookmarkOfVisited67334": "1",
    "captcha_webworker_supported": "2",
    "pgv_pvi": "867511689741213346",
    "_BEACON_0WEB0OEX9Y4SQ244_4.5.17-web_direct_log_id": "1004",
    "__BEACON_0WEB0OEX9Y4SQ244_beacon_config": "",
    "__BEACON_0WEB0OEX9Y4SQ244_beacon_config_request_time": "1697595580073",
    "__BEACON_0WEB0OEX9Y4SQ244_is_new_user": "false",
    "__BEACON_0WEB0OEX9Y4SQ244_last_report_time": "1697595600886",
    "__BEACON_0WEB0OEX9Y4SQ244_logid_start": "1685427433647",
    "__BEACON_0WEB0OEX9Y4SQ244_sending_event_ids": "[]",
    "__BEACON_0WEB0OEX9Y4SQ244_session_storage_key": JSON.stringify({
        "sessionId": "0WEB0OEX9Y4SQ244_1697595580073",
        "sessionStart": 1697595580073
    }),
    "__BEACON_deviceId": "rDByDs5hWxY1dYCmTTQkD4SY93K9nTQh"
...
function get_collect(ans) {
    ...
    let offsetX = 0
    for (let i = 0; i <= 45; i++) {
        window.mousemove({"pageX": offsetX, "pageY": 285 + parseInt(Math.random() * 5)})
        offsetX += parseInt(Math.random() * 10)
    }
    console.log(ans)
    // window.TDC.setData({"dragEl": [{"data": ans, "elem_id": 1, "type": "DynAnswerType_POS"}]})
    window.TDC.setData({ft: "qf_7P___H"})
    result = decodeURIComponent(window.TDC.getData(!0))
    return result
}

app.post('/get_collect', function (req, res) {
    let body = req.body;
    let ans = body.ans;
    let result = get_collect(ans)
