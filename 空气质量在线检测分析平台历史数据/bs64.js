...
function encrypt1(a1d3,mGAbUtDKV,cU6TA,city){
    t = new Date().getTime()
    ok72GS = {"city":city}
    q1 = BASE64.encrypt(JSON.stringify({
            appId: a1d3,
            method: mGAbUtDKV,
            timestamp: t,
            clienttype: cU6TA,
            object: ok72GS,
            secret: hex_md5(a1d3 + mGAbUtDKV + t + cU6TA + JSON.stringify(osv8SPtGs1(ok72GS)))
        }
        ))
    return q1
}

function encrypt(a1d3,mGAbUtDKV,cU6TA,city,key,iv){
    t = new Date().getTime()
    ok72GS = {"city":city}
    q1 = BASE64.encrypt(JSON.stringify({
            appId: a1d3,
    ...
    return DES.encrypt(q1, key, iv)
}

function encrypt2(a1d3,mGAbUtDKV,cU6TA,city,key,iv){
    t = new Date().getTime()
    ok72GS = {"city":city}
    q1 = BASE64.encrypt(JSON.stringify({
            appId: a1d3,
...
function dataJson(data,key1,iv1,key2,iv2) {
    data = BASE64.decrypt(data);
    data = DES.decrypt(data, key1, iv1);
    data = AES.decrypt(data, key2, iv2);
    console.log(data)
    data = BASE64.decrypt(data);
    return data;
}